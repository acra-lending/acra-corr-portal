...
function TrainingTools({menuItems, trainingItems}){
    ...
    const URL = `${process.env.NEXT_PUBLIC_API_URL}/api/corr-training-tools?pagination[page]=${pageNumber}&pagination[pageSize]=6&populate=*`;
    const {data} = useSWR(URL, 
        fetcher,
        {
            fallbackData: trainingItems
        });
    return(
                            ...
                            <table className="border-collapse table-fixed w-full text-sm">
                                ...
                                <tbody className="bg-white dark:bg-slate-800">
                                    {data.data.map((item, key)=>{
                                       return <tr key={key}>
                                            <td className="border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400">{item.attributes.trainingTitle}</td>
...
export async function getServerSideProps(){
    const [menuResponse, trainingRespnse] = await Promise.all([
        ...
        fetch(`${process.env.BASE_URL}/corr-training-tools?pagination[page]=1&paginatoin[pageSize]=6&populate=*`)
    ]);

    const [menuItems, trainingItems] = await Promise.all([
        menuResponse.json(),
        trainingRespnse.json()
    ])
    return{ props: {menuItems, trainingItems}}
}
